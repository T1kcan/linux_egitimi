...
- Create a `text` file named `btk.txt`.
​
```bash
cat << EOF > btk.txt
btktest:devops path
...
- Display the content of btk.txt.
​
```bash
cat btk.txt
```

- In the content of btk.txt, replace or translate aer letters with 'QAZ'.
​
```bash
cat btk.txt | tr aerp QAZM
```
​
...
```
​
- Delete all the vowels in the content of btk.txt.
​
```bash
cat btk.txt | tr -d aeiou
```
​
- Write the whole content of btk.txt in capital letters.
​
```bash
cat btk.txt | tr [a-z] [A-Z]
```
...
​
- Display days.txt or write 'btk' on the screen, then write 'one'.
​
```bash
cat days.txt || echo btk ; echo one
```
...
- EXERCISE:
​
  1.a. Search for “btk.doc” in the current directory
    b. If it exists display its content
    c. If it does not exist print message “Too early!”
  2.Create a file named “btk.doc” that contains “Congratulations”
  3.Repeat Step 1
​
```bash
********************************
