...
```bash
#!/bin/bash
set -x # allow us to see the commands&outputs on the screen to debug purposes  
echo "hello"
date
pwd
ls
...
- The following examples are valid variable names. (It is best practice to use variables in capital letters)
...
- $? is to gather status of previos command/script status, it's the exit status of the last command. For instance; '0' is successful, '127' command not found, '1' general error.
